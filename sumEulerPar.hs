...
sumTotientChunk :: Integer -> Integer -> Int
sumTotientChunk lower upper = sum (map euler [lower, lower+1 .. upper] `using` parListChunk (fromInteger (upper-lower) `div` 10) rdeepseq)
...
main = do args <- getArgs
          ...
          hPutStrLn stderr ("Sum of Totients between [" ++ 
                      (show lower) ++ ".." ++ (show upper) ++ "] is " ++ 
                       show (sumTotientChunk lower upper))
